...
def crawl_reviews(driver, max_reviews=2000):
    ...
    for num, review in enumerate(reviews, start=1):
        ...
        try:
            ...
            try:
                option1 = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > div > div > div:nth-child(1) > p > span:nth-child(1)").text
            except Exception:
                option1 = ""
            print(f"[DEBUG] 옵션1: {option1}")

            try:
                option2 = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > div > div > div:nth-child(1) > p > span:nth-child(2)").text
            except Exception:
                option2 = ""
            print(f"[DEBUG] 옵션2: {option2}")

            try:
                size = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > div > div > div.css-1wzypsy.eimmef70 > div:nth-child(1) > p").text
            except Exception:
                size = ""
            print(f"[DEBUG] 선택 사이즈: {size}")

            try:
                quality = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > div > div > div.css-1wzypsy.eimmef70 > div:nth-child(2) > p").text
            except Exception:
                quality = ""
            print(f"[DEBUG] 퀄리티: {quality}")

            try:
                color = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > div > div > div.css-1wzypsy.eimmef70 > div:nth-child(3) > p").text
            except Exception:
                color = ""
            print(f"[DEBUG] 색감: {color}")

            try:
                height = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > div > div > div:nth-child(3) > p > span:nth-child(1)").text
            except Exception:
                height = ""
            print(f"[DEBUG] 고객 신장: {height}")

            try:
                weight = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > div > div > div:nth-child(3) > p > span:nth-child(2)").text
            except Exception:
                weight = ""
            print(f"[DEBUG] 고객 중량: {weight}")

            try:
                customer_size = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > div > div > div:nth-child(3) > p > span:nth-child(3)").text
            except Exception:
                customer_size = ""
            print(f"[DEBUG] 고객 사이즈: {customer_size}")

            # "더보기" 버튼 처리 및 리뷰 내용 추출
            try:
                more_button = driver.find_element(By.CSS_SELECTOR, f"{base_selector} > div:nth-child(3) > span > p")
                driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", more_button)
                time.sleep(1)
                more_button.click()
                time.sleep(1)
            except Exception:
                print(f"[DEBUG] 리뷰 {num}: 더보기 버튼 없음")
...
def save_to_csv(data, product_id):
    ...
    filename = os.path.join(folder_path, f"슬리브리스_p_1_{product_id}_112538672.csv")
    # filename = os.path.join(folder_path, f"test_{product_id}_2000개.csv")
